...
ActiveAdmin.register ImportJob do
  ...
  controller do
    def create
      ...
      redirect_to admin_import_job_path(@import_job), 
  ...
  # Customize filters
  filter :webstore, collection: proc { Webstore.order("LOWER(name) asc") }
  filter :filename
  ...
  action_item only: :show do
    link_to('Submit Import Job', submit_admin_import_job_path(import_job), method: :post, confirm: "You are about to import all the Import Job items. Continue?")
  end
  ...
  index do
    ...
    column :filename, sortable: :filename do |import_job|
      link_to import_job.filename, admin_import_job_path(import_job)
    end
    column "Items" do |import_job|
      import_job.import_job_items.count
      link_to "#{import_job.import_job_items.count} [view items]", admin_import_job_import_job_items_path(import_job)
    end
    column :webstore
  ...
  member_action :submit, method: :post do
    ...
    if job.nil?
      redirect_to admin_import_job_path(job), alert: "Unable to find ImportJob."
      return
    end
